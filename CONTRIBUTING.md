...
Jesse files issues on `jessepollak/home` with a kind prefix in the title (`product(...)`, `design(...)`, `feat(...)`, `fix(...)`, `test(...)`, `ops(...)`, `dx(...)`, `docs(...)`, `chore(...)`); adding `factory` means start working on the issue. The factory bot follows the run triggers and delivery loop in the [operating manual](docs/operating-manual.md), marks the issue `factory:working`, hands off with `factory:review` (a PR whose checks are green, on the PR) or `factory:needs-jesse` (anything that needs Jesse's decision), works on `agent/<issue>`, and delivers a normal PR ending with `Closes #<issue>` for implementation or `Refs #<issue>` for design proposals (or an issue comment for `product(...)` research). Any Jesse issue/PR comment or PR review triggers a follow-up; re-adding `factory` triggers a comment-less look-again run.

...
- [ ] Captured from the final current PR head after the final source-code push (not comps, empty scaffold, or unlabeled `/dev` harness)
- [ ] New immutable per-capture manifest records head/base SHA, capture time, origin/route, browser, viewport/DPR, fixture-vs-live boundary, command/script, and SHA-256 file hashes; old provenance is never overwritten or relabeled
...
- [ ] Stale tip/base/HOLD text replaced or explicitly superseded; published exact-tip proof reviewed before advancing to `status:ready-for-review` or `status:needs-jesse` for merge
- [ ] After every source-code push, I will make fresh captures, publish new GitHub `user-attachments` files, create new immutable per-capture manifests, and obtain renewed proof review

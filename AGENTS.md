...
Crew roles, labels, sequencing, capacity, risk-based QA, daily domain quality reviews, proof bar, merge policy, and docs policy: [docs/operating-manual.md](docs/operating-manual.md).
...
- Use native todos only for a short [current-action checklist](docs/operating-manual.md#session-execution-checklist), with one parent action `in_progress`; GitHub remains the sole backlog and the agent fleet tracks parallel children. For explicitly authorized coordination, follow the [delegated capacity, QA, review, and publication gates](docs/operating-manual.md#continuous-issue-pipelines); do not duplicate that operating policy here.

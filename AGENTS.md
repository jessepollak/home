...
- **Bootstrap and factory live mode.** Git does not copy `apps/web/.env.local`. For ordinary worktrees, copy the primary checkout's file with `install -m 600` only when needed; never overwrite, print, or commit it. Isolated runs remain credential-free, while the provisioned studio factory runner follows [Factory live mode](docs/browser-validation.md#factory-live-mode) with per-slot verifier state and policy-bounded preview authority.

...
- **Execution modes.** Tracking, local interactive authorization, and standalone queue eligibility are distinct. A local session works on a named issue only on Jesse's explicit current-session instruction. The standalone queue may select only an owner-authored eligible leaf with a Jesse-applied `factory:ready`, `status:todo`, and exactly one lane and priority; its implementation lives outside Home. See [operating manual — Execution modes](docs/operating-manual.md#execution-modes).

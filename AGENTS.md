...
- **Execution modes.** Tracking, local interactive authorization, and operator-invoked factory execution are distinct. A local session works on a named issue on Jesse's explicit current-session instruction. `bun run factory:run <issue>` is authorized by its operator invocation and fails closed unless the issue is open, owner-authored, `status:todo`, has exactly one lane and priority, and has no open PR. See [operating manual — Execution modes](docs/operating-manual.md#execution-modes).

...
- **One command.** `bun check` runs test, lint, typecheck, and build for `apps/web`. Bun `1.3.12`, pinned by `packageManager`. Install with `--frozen-lockfile`; if `bun.lock` moves, the toolchain is wrong.
...
- **Design-system lint only gets stricter.** `apps/web/eslint-suppressions.json` baselines existing `shadcn/no-restyle` violations. Do not regenerate or increase its counts; after removing violations, run `bun run --cwd apps/web lint:prune`.
...
- **Money-loop gates.** Do not restate them from memory: [Home-is-thin invariants](docs/home-is-thin.md). The durable flow is `apps/web/server/actions/` (prepare, confirm, handle, list); calldata builders remain in `apps/web/server/money-actions/{issue,prepare-send}.ts`.

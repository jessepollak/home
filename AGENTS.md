...
- Jesse owns product intent, consequential decisions, privileged actions, final approval, and merge. The factory owns issue refinement, implementation coordination, independent review, evidence, and pull-request delivery; an operator invocation authorizes only the named factory run.
...
- **Execution modes.** Tracking, local interactive authorization, and operator-invoked factory execution are distinct. A local session works on a named issue on Jesse's explicit current-session instruction. `bun run factory:run <issue>` is authorized by its operator invocation and fails closed unless the issue is an open, owner-authored leaf with a native parent, `status:todo`, exactly one lane and priority, and no open PR. See [operating manual — Execution modes](docs/operating-manual.md#execution-modes).
- End every new factory-authored public comment, thread reply, review, and PR body with `<!-- factory -->`; an issue body an agent creates on its own initiative—not at Jesse's direct request—uses the same marker. The review workflow recognizes legacy `<!-- hugo -->` text only for compatibility.

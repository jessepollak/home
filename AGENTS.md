...
- **Harness delegation.** Follow the [harness delegation contract](docs/operating-manual.md#harness-delegation). Factory coordination never displaces the delegating session's scope, decision, integration, or final-acceptance authority.
...
- On pickup, the factory applies `factory:working`, comments a start notice, and works in `agent/<issue>`. Implementation pull requests (`feat`, `fix`, `test`, `ops`, `dx`, `docs`, and `chore`) end with `Closes #<issue>`; design proposals and product follow-up pull requests end with `Refs #<issue>`, while product research may instead produce an issue comment.
...
- Factory PRs stay draft until handoff; the factory marks a PR ready and requests Jesse's review only after CI is green. Commits are authored by `jessepollakj`. Jesse alone approves and merges; `main` requires one approving review plus CODEOWNERS.
...
- **Browser validation.** Every user-visible UI or core-flow implementation follows [Verify a change](docs/browser-validation.md#verify-a-change) and the browser-validation contract: map the diff to feature-map surfaces and required rungs, use pinned `agent-browser` before and after editing, and keep Playwright as the sole committed automated browser regression layer.
- **Storybook design review.** Follow the [issue's design scope](.agents/skills/design-engineering/SKILL.md#follow-the-issue-scope) and the [review-board workflow](docs/design-system.md#review-boards) for proposals, review, and selection. Exploration stops at Jesse's selection; adoption uses the same components and [Home verification](docs/ui-pr-previews.md). The board does not change approval authority.
- **Design handoff coverage.** A design or library handoff maps every follow-up to one durable disposition; the [follow-up directory](docs/design-explorations/design-system/follow-ups/) is the library's single logical coverage table, with one file per numbered follow-up. Its reviewer checks coverage and design/code parity. Follow [design handoff coverage](docs/operating-manual.md#design-handoff-coverage).
- **Storybook MCP workshop.** In an interactive shell run `export STORYBOOK_PORT=6006` and start Storybook with `bun run --cwd apps/web storybook` before relying on its tools; the root `.mcp.json` registers `http://127.0.0.1:${STORYBOOK_PORT}/mcp` as `storybook`. Discover owned components with the MCP docs tools (`docs-list`, `docs-show`) instead of re-inventing them, compose page-level flows as journey stories under `apps/web/stories/journeys/`, run `bun run --cwd apps/web test:stories` (or MCP `test-run`) so `play` functions and a11y checks execute, then capture proof. Stop the dev server before a full `test:stories` run; they share the Storybook Vite cache.
...
- **Shared design docs and registries grow by adding one file per entry**, not by appending entries to a shared list or table. Keep stable guidance in the shared doc and link to the entry directory.

...
export async function removeStatusLabels(plan, options) {
  ...
  for (const label of plan.labelsToRemove) {
    ...
    const endpoint = new URL(
      `/repos/${repository}/issues/${plan.issueNumber}/labels/${encodeURIComponent(label)}`,
      apiUrl,
    );
    ...
    headers.set("authorization", ["Bearer", options.token].join(" "));
...
async function main() {
  ...
  const plan = planStatusLabelChanges(eventName, payload);
  const removed = await removeStatusLabels(plan, {
    repository: expectedRepository,
    apiUrl: process.env.GITHUB_API_URL,
    token: process.env.GITHUB_TOKEN,
  });
  console.log(JSON.stringify({ ...plan, removed }));
}

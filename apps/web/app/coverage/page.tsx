...
function CoverageMap() {
  ...
  const configured = new Map(coverageRegistry.map((record) => [record.countryCode, record]));
  return (
    <figure className="overflow-hidden rounded-lg border bg-muted/30 p-3">
      <svg viewBox="0 0 720 360" role="img" aria-labelledby="coverage-map-title coverage-map-description" className="h-auto w-full">
        ...
        <desc id="coverage-map-description">Dots link to country details. Grey dots are not configured; green and amber dots show documented and conditional issuer-route research. Map position is not eligibility.</desc>
        <rect width="720" height="360" rx="8" fill="currentColor" opacity="0.04" />
        {Object.entries(coordinates).map(([code, [longitude, latitude]]) => {
          const record = configured.get(code as (typeof coverageRegistry)[number]["countryCode"]);
          const x = ((longitude + 180) / 360) * 720;
          const y = ((90 - latitude) / 180) * 360;
          const dot = <circle cx={x} cy={y} r={record ? 4.5 : 1.7} fill={record ? issuerColors[record.issuerRoute.status] : "#a3a3a3"} opacity={record ? 1 : 0.5} />;
          return record ? <a key={code} href={`#country-${code}`} aria-label={`${presentationRegions[record.countryCode].countryName}: ${issuerLabels[record.issuerRoute.status]}`}>{dot}</a> : <g key={code} aria-hidden="true">{dot}</g>;
        })}
      </svg>
      <figcaption className="mt-2 text-sm text-muted-foreground">Natural Earth v5.1.2 label points. Neutral dots are unconfigured and do not inherit status from a shared currency.</figcaption>
    </figure>
  );
}
...
export default async function CoveragePage({ searchParams }: PageProps<"/coverage">) {
  ...
  const records = sortCoverage(coverageRegistry.filter((record) => {
    ...
    const searchable = `${region.countryName} ${record.countryCode} ${record.currencyCode} ${region.currency.name} ${region.candidateAsset?.symbol ?? ""}`.toLocaleLowerCase();
    ...
    <main className="mx-auto flex w-full max-w-7xl flex-col gap-10 px-4 py-8 sm:px-6 lg:px-8">
      <header className="flex flex-col gap-4">
        ...
        <dl className="grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-lg border p-4"><dt className="text-sm text-muted-foreground">Configured countries</dt><dd className="text-3xl font-semibold">{coverageRegistry.length}</dd><p className="text-xs text-muted-foreground">Home region configuration · checked {COVERAGE_REGISTRY_CHECKED_AT}</p></div>
        </dl>
      </header>
      ...
      <section aria-labelledby="countries-heading" className="space-y-4">
        <div><h2 id="countries-heading" className="text-2xl font-semibold">Countries</h2><p className="text-sm text-muted-foreground">GDP is {coverageGdpSnapshot.indicatorName}, indicator {coverageGdpSnapshot.indicator}, reference year {coverageGdpSnapshot.year}. Missing figures remain listed and sort last.</p></div>
        ...
        <p aria-live="polite" className="text-sm text-muted-foreground">Showing {records.length} of {coverageRegistry.length} countries.</p>
          ...
          <table className="w-full min-w-3xl border-collapse text-left text-sm">
            ...
            <tbody>{records.map((record) => {
              ...
              return <tr key={record.countryCode} id={`country-${record.countryCode}`} className="border-t align-top">
                <th scope="row" className="p-3"><details><summary className="font-semibold">{region.countryName} <span className="font-normal text-muted-foreground">{record.countryCode}</span></summary><dl className="mt-3 space-y-1 font-normal"><div><dt className="inline font-medium">Local rail: </dt><dd className="inline">{record.issuerRoute.rail}</dd></div><div><dt className="inline font-medium">Audience: </dt><dd className="inline">{record.issuerRoute.audience}</dd></div><div><dt className="inline font-medium">Provider identity: </dt><dd className="inline">{record.homeRoute.providerId ?? "None"}</dd></div><div><dt className="inline font-medium">Payment method identities: </dt><dd className="inline">{record.homeRoute.paymentMethodIds.join(", ") || "None"}</dd></div><div><dt className="inline font-medium">Quote observation: </dt><dd className="inline">None recorded</dd></div></dl></details></th>
                <td className="p-3">{record.currencyCode}<br /><span className="text-muted-foreground">{region.candidateAsset?.symbol} · {region.candidateAsset?.issuer}</span></td>
      ...
      <footer className="border-t pt-6 text-sm text-muted-foreground"><p>Issuer evidence: <a className="underline" href={COVERAGE_RESEARCH_SOURCE.url}>{COVERAGE_RESEARCH_SOURCE.name}</a>, checked {COVERAGE_RESEARCH_SOURCE.checkedAt}. GDP: <a className="underline" href={coverageGdpSnapshot.sourceUrl}>World Bank</a>, {coverageGdpSnapshot.year}, snapshot downloaded {coverageGdpSnapshot.downloadedAt}. Map: Natural Earth v5.1.2 public-domain label points.</p></footer>
    </main>
  );
}

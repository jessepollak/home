...
test("misconfigured enabled ingress reports unavailable without recording secrets", async () => {
...
test("enabled ingress rejects signed topic mismatch, accepts matching topic, and logs only safe codes", async () => {
  ...
  const logs: string[] = [];
  setObservabilityLogWriterForTests((line) => { logs.push(line); });
  const store = spyOn(storeModule, "createCardEventStore").mockReturnValue({ insert: async (event) => { stored.push(event); return true; } });
  ...
  try {
    ...
    const accepted = await POST(request(body, "payment-updated", headers), context("payment-updated"));

...
const enabled = {
  BRIDGE_ENABLED: "1",
  BRIDGE_MODE: "sandbox",
  BRIDGE_STRIPE_API_VERSION: "2026-08-27.basil",
...
describe("Bridge configuration https://apidocs.bridge.xyz/get-started/introduction/quick-start/setting-up-sandbox", () => {
  test("is inert by default and pins mode, origin, chain and USDC", () => {
    ...
    expect(readBridgeConfig({ ...enabled, BRIDGE_STRIPE_API_VERSION: "2026-08-27" })?.stripeApiVersion).toBe("2026-08-27");
  ...
  test("fails closed on incomplete config and invalid spender", () => {
    ...
    for (const version of ["2026-13-27.basil", "2026-02-30.basil", "2026-08-27.", "v0", "2026-08-27.Basil"]) {
      expect(() => readBridgeConfig({ ...enabled, BRIDGE_STRIPE_API_VERSION: version })).toThrow();
    }

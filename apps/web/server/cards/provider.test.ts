...
const bridge = readBridgeConfig({ BRIDGE_ENABLED: "1", BRIDGE_MODE: "sandbox", BRIDGE_STRIPE_API_VERSION: "2026-08-27.basil", BRIDGE_WEBHOOK_PUBLIC_KEY: pem,
...
function stripeDelivery(timestamp = Math.floor(now / 1000)): Delivery {
  const raw = encode({ id: "evt_fixture", type: "issuing_authorization.created", api_version: "2026-08-27.basil", livemode: false, created: timestamp, data: { object: {
...
const adapters: { name: string; expectedStale: "rejected" | "stale"; make(): CardProvider; delivery(): Delivery; staleDelivery(): Delivery; disabled(): boolean; expected: CardObservation }[] = [
  ...
  { name: "Bridge (https://apidocs.bridge.xyz/platform/additional-information/webhooks/signature)", expectedStale: "stale", make: () => createBridgeWebhookProvider(bridge, () => now),
...
for (const adapter of adapters) describe(`${adapter.name} provider seam`, () => {
  ...
  test("rejects bad signatures and does not store stale deliveries", async () => {
    const provider = adapter.make();
    const stored: CardObservation[] = [];
    const handler = createCardWebhookHandler(provider, { insert: async (event) => { stored.push(event); return true; } });
    const delivery = adapter.delivery();
    expect(await handler(new Uint8Array([...delivery.raw, 32]), delivery.headers, delivery.topic)).toBe("rejected");
    const stale = adapter.staleDelivery();
    expect(await handler(stale.raw, stale.headers, stale.topic)).toBe(adapter.expectedStale);
    expect(stored).toEqual([]);
    expect(await handler(delivery.raw, delivery.headers, delivery.topic)).toBe("accepted");
    expect(stored).toEqual([adapter.expected]);
  });

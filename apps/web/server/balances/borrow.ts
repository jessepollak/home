...
type Dependencies = {
  readSnapshot?: MorphoMarketRpcReader["readSnapshot"];
...
export function createBorrowPositionsReader(dependencies: Dependencies = {}) {
  ...
  let reader: MorphoMarketRpcReader["readSnapshot"] | null = dependencies.readSnapshot ?? null;

  return async function readBorrowPositions(
    owner: PortfolioAddress,
    at: MorphoPinnedBlock,
  ): Promise<BorrowRead> {
    const signal = AbortSignal.timeout(deadlineMs);
    return {
      markets: await Promise.all(markets.map(async (market): Promise<BorrowMarketRead> => {
        const marketId = market.marketId.toLowerCase() as BorrowMarketKey;
        try {
          reader ??= createMorphoMarketRpcReader().readSnapshot;
          const snapshot = await reader(owner, market, signal, at);
          return {
            marketId,
            status: "ready",
          ...
          return { marketId, status: "unavailable" };
        }
      })),
    };
  };
...
export function borrowReadComplete(read: BorrowRead | null | undefined): boolean {
  return Boolean(read) && read!.markets.every((market) =>
    ...
    BORROW_MARKETS.every((market) =>
      read!.markets.some((entry) => entry.marketId === market.marketId.toLowerCase()));
}
...
export function borrowPricingPairs(read: BorrowRead | null | undefined): PricingPair[] {
  return (read?.markets ?? []).flatMap((entry): PricingPair[] => {
    ...
    if (emptyPosition(entry)) return [];
    const market = getBorrowMarketRef(entry.marketId);
    if (!market) return [];
    return [{
      marketId: entry.marketId,
      borrowAprWad: entry.borrowAprWad,
...
export function assembleBorrow(
  read: BorrowRead | null | undefined,
  pairs: readonly PricingPair[],
  price: (holding: ReadHolding) => Holding,
): BalancesBorrow {
  return {
    ...
    positions: pairs.map((pair): BorrowPosition => {
      ...
      return {
        ...
        debt: {
          ...
          marketId: pair.marketId,
          asset: {
            key: erc20AssetKey(pair.debt.contractAddress!),
            name: pair.debt.name,
            symbol: pair.debt.symbol,
            decimals: pair.debt.decimals,
          },
          balance: { status: "ready", baseUnits: pair.debt.balance.baseUnits! },
          value: debt.value,
        },
...
function pricingHolding(
  asset: BorrowMarketRef["collateralToken"],
  id: string,
  baseUnits: string,
): ReadHolding {
  ...
  return {
    ...
    kind: "erc20",
    source: "borrow",
    name: registry?.name ?? asset.name,
    symbol: registry?.symbol ?? asset.symbol,

...
const base: FundingReservation = {
  ...
  customerRef: null, creationBlock: "100", createdAt: "2026-09-12T00:00:00.000Z",
};
...
describe("MemoryFundingOrderStore contract", () => {
  ...
  test("enforces unique provider IDs and unique receipt claims", async () => {
    ...
    await store.completeDispatch(base.id, { providerOrderId: "provider-1", expectedTokenAmountAtomic: "2000000", fees: [], expiresAt: null, instructions: { kind: "qr", scheme: "qris", payload: "payload", amount: "20000.00", currency: "IDR" }, updatedAt: "2026-09-12T00:00:01.000Z" });
    const second = { ...base, id: "22222222-2222-4222-8222-222222222222", intentDigest: "other" };
    await store.reserve(second);
    await expect(store.completeDispatch(second.id, { providerOrderId: "provider-1", expectedTokenAmountAtomic: "2000000", fees: [], expiresAt: null, instructions: { kind: "redirect", url: "https://example.com" }, updatedAt: "2026-09-12T00:00:01.000Z" })).rejects.toThrow("funding-provider-order-conflict");
    expect(await store.claimReceipt(base.id, { transactionHash: `0x${"1".repeat(64)}`, logIndex: 2, updatedAt: "2026-09-12T00:00:02.000Z" })).not.toBeNull();
    expect(await store.claimReceipt(second.id, { transactionHash: `0x${"1".repeat(64)}`, logIndex: 2, updatedAt: "2026-09-12T00:00:02.000Z" })).toBeNull();

...
3. **VA response and fees.** The documented VA shape (`virtualAccountName`, `virtualAccountNo`, `expiredDate`, `baseAmount`, `amount`, `fees[]`) matches IDRX's `createPayment`, but `amount` and `baseAmount` are JSON **numbers**; `fees[].amount` is a string. `readDecimal` accepts both. A VA channel requires a bank account for that bank registered on the IDRX account; otherwise the create returns `400 { data: { code: "BANK_ACCOUNT_REQUIRED", requiredBankChannel } }` and no order exists. Not yet exercised live (the proof account had no Mandiri/BRI bank account registered).

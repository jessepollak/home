...
describe("Ripio funding adapter", () => {
  ...
  test("verifies the raw webhook body before returning a provider order ID", () => {
    ...
    expect(ripioProvider.onramp!.verifyWebhook!(raw, new Headers({ "http-x-wh-signature-256": signature }), context(fetch))).toEqual({ providerOrderId: "44444444-4444-4444-8444-444444444444" });
    expect(ripioProvider.onramp!.verifyWebhook!(raw, new Headers({ "http-x-wh-signature-256": "0".repeat(64) }), context(fetch))).toBeNull();
  });
});

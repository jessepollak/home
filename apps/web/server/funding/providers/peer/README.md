...
- **Current claim (September 22, 2026):** production is enabled for the US and GB Peer corridors on `home.jesse.xyz` by operator decision and is being validated under the verification ladder's caps (Rung 2 today, a $1 Rung 3 round trip once the verifier supports the cash-out confirm). The staging proof was deliberately skipped (see G2), and Peer's written corridor confirmation remains outstanding. No funded acceptance claim is made yet.
...
- **G3 production:** production is enabled for US/USD (Cash App, Zelle) and GB/GBP (Monzo, Revolut) on `home.jesse.xyz` as of 2026-09-22 and is being validated by the verifier ladder in `docs/browser-validation.md` (Rung 2 today, a $1 Rung 3 round trip once the verifier supports the cash-out confirm). Peer's written country/corridor confirmation for every enabled row remains outstanding and must be attached here when received; until then this is not a provider-confirmed corridor.

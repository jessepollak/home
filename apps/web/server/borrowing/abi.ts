...
export type BorrowMoneyActionCall = MoneyActionCall & {
  approval?: { assetId: string; spender: BorrowAddress };
};
...
export function encodePosition(marketId: `0x${string}`, account: BorrowAddress) { return data(SELECTOR.position, bytes32Word(marketId), addressWord(account)); }
...
export function encodeBorrowRateView(ref: BorrowMarketRef, market: readonly bigint[]) {
...
export function encodeBalanceOf(account: BorrowAddress) { return data(SELECTOR.balanceOf, addressWord(account)); }
export function encodeAllowance(owner: BorrowAddress, spender: BorrowAddress) { return data(SELECTOR.allowance, addressWord(owner), addressWord(spender)); }

export function approveCall(token: { address: BorrowAddress; id: string }, spender: BorrowAddress, amount: bigint): BorrowMoneyActionCall {
...
export function supplyCollateralCall(ref: BorrowMarketRef, amount: bigint, owner: BorrowAddress): MoneyActionCall {
...
export function borrowCall(ref: BorrowMarketRef, amount: bigint, owner: BorrowAddress): MoneyActionCall {
...
export function repayCall(ref: BorrowMarketRef, amount: bigint, owner: BorrowAddress): MoneyActionCall { return repayAssetsOrSharesCall(ref, amount, BigInt(0), owner); }
export function repaySharesCall(ref: BorrowMarketRef, shares: bigint, owner: BorrowAddress): MoneyActionCall { return repayAssetsOrSharesCall(ref, BigInt(0), shares, owner); }
export function withdrawCollateralCall(ref: BorrowMarketRef, amount: bigint, owner: BorrowAddress): MoneyActionCall {
...
export function encodeCoinbaseExecuteBatch(calls: readonly MoneyActionCall[]): `0x${string}` {
  ...
  const tupleBodies = calls.map((call) => {
    ...
    return [addressWord(call.to as BorrowAddress), uintWord(BigInt(call.value)), uintWord(BigInt(96)), uintWord(BigInt(callData.length / 2)), paddedData].join("");
  });
...
function repayAssetsOrSharesCall(ref: BorrowMarketRef, assets: bigint, shares: bigint, owner: BorrowAddress): MoneyActionCall {
...
function marketParamWords(ref: BorrowMarketRef) { return [addressWord(ref.loanToken.address), addressWord(ref.collateralToken.address), addressWord(ref.oracle), addressWord(ref.irm), uintWord(ref.lltvWad)]; }
function data(selector: string, ...words: string[]): `0x${string}` { return `0x${selector}${words.join("")}`; }
function addressWord(address: BorrowAddress) { if (!/^0x[0-9a-fA-F]{40}$/.test(address)) throw new TypeError("Invalid address."); return address.slice(2).toLowerCase().padStart(64, "0"); }

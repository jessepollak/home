    ...
    >
      ...
      <MoneyModalBody className="pt-4">
        <dl>
          {rows.map((row) => (
            <div
              className="relative grid min-h-11 grid-cols-[minmax(6rem,0.65fr)_minmax(0,1.35fr)] items-center gap-3 text-sm after:absolute after:inset-x-0 after:bottom-0 after:h-px after:bg-border last:after:hidden"
              key={row.label}
            >

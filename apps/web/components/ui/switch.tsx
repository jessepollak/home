  ...
  return (
    <SwitchPrimitive.Root
      ...
      className={cn(
        ...
        // The absolute `::before` is inset relative to the 26px padding box
        // (28px minus the 1px borders), so it needs a 9px overhang on each
        // side to reach the 44px target.
        "relative inline-flex h-7 w-12 shrink-0 cursor-pointer items-center rounded-full border border-input bg-input p-1 transition-colors outline-none before:absolute before:inset-x-0 before:-inset-y-2.25 before:content-[''] md:before:content-none data-checked:border-primary data-checked:bg-primary focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 disabled:cursor-not-allowed disabled:opacity-50 motion-reduce:transition-none",
        className
      )}
      {...props}
    >

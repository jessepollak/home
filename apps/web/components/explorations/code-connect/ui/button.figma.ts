...
export default {
  example: figma.code`<Button variant="${variant}" size="${size}"${state === 'disabled' ? figma.code` disabled` : null}${state === 'loading' ? figma.code` aria-busy` : null}>${showIcon ? iconExample : null}{${JSON.stringify(label)}}</Button>`,

...
export default { example: figma.code`<MoneyBreakdownLegend items={[{ id: "${kind}", label: "${label}", value: ${JSON.stringify(amount)}, weight: 1 }]} />`, imports: ['import { MoneyBreakdownLegend } from "@/components/signed-balance-bar"'], id: 'money-breakdown-item', metadata: { nestable: true } }

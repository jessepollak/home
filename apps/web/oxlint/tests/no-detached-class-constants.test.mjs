// Behavioral tests for the no-detached-class-constants rule. Every case runs
// through the pinned Oxlint binary in an isolated project; TypeScript wrappers
...
function lint(code, extension = "tsx") {
  const fixture = path.join(mirror, "client", `fixture.${extension}`);
  return writeFile(fixture, code).then(() => {
    const result = spawnSync(
      path.join(appsWebDir, "node_modules", ".bin", "oxlint"),
      ["-c", ".oxlintrc.jsonc", "--disable-nested-config", "-f", "json", `client/fixture.${extension}`],
...
const expectedMessage = {
  detached: "resolves to a static class string",
  detachedAggregate: "is a local static class map whose lookup resolves to class strings",
};
    ...
    describe(name, () => {
      ...
      cases.invalid.forEach((entry, index) => {
        it(`invalid ${index + 1}`, async () => {
          const diagnostics = await lint(entry.code, entry.extension);
          expect(diagnostics).toHaveLength(entry.errors.length);
          for (const [errorIndex, expected] of entry.errors.entries()) {
            const message = diagnostics[errorIndex]?.message ?? "";
            expect(message).toContain(expected.data.name);
...
// TypeScript wrappers run through the same pinned Oxlint parser as production;
// no ESLint parser or compatibility implementation participates.
ruleTester.run("no-detached-class-constants (TypeScript)", noDetachedClassConstantsRule, {
  valid: [
...
// Oxlint regressions beyond PR #673's 96-case corpus. Angle-bracket assertions
// use .ts because TypeScript reserves that syntax for JSX in .tsx.
ruleTester.run("no-detached-class-constants (Oxlint regressions)", noDetachedClassConstantsRule, {
  valid: [
    // Mutable aggregate shape and value bindings are not static class maps,
    // even without a recorded write.
    "let map = { active: \"flex\" };\nfunction Row({ status }) { return <div className={map[status]} />; }",
    "let key = \"active\";\nconst map = { [key]: \"flex\" };\nfunction Row({ status }) { return <div className={map[status]} />; }",
    "let value = \"flex\";\nconst map = { active: value };\nfunction Row({ status }) { return <div className={map[status]} />; }",
    // Calls do not become static aggregates, including wrappers that return an
    // otherwise literal object.
    "const map = Object.freeze({ active: \"flex\" });\nfunction Row({ status }) { return <div className={map[status]} />; }",
    // Writes after the class use still make the whole binding dynamic.
    "let tone = \"flex\";\nfunction Row() { return <div className={tone} />; }\ntone = \"hidden\";",
    "let map = { active: \"flex\" };\nfunction Row({ status }) { return <div className={map[status]} />; }\nmap = { active: \"hidden\" };",
    // Const aggregate bindings remain mutable. Direct, computed, and aliased
    // member writes invalidate the map regardless of traversal order.
    "const map = { active: \"flex\" };\nfunction Row() { return <div className={map.active} />; }\nmap.active = getTone();",
    "const map = { active: \"flex\" };\nfunction Row() { return <div className={map.active} />; }\nmap[\"active\"]++;",
    "const map = { active: \"flex\" };\nconst alias = map;\nfunction Row() { return <div className={map.active} />; }\ndelete alias.active;",
    // Assignment-target patterns and loop targets are writes too. These cover
    // nested property/default and array/rest targets plus a bounded alias.
    "const map = { active: \"flex\" };\nfunction Row() { return <div className={map.active} />; }\n({ tone: map.active = \"fallback\" } = source);",
    "const map = { active: \"flex\" };\nconst alias = map;\nfunction Row() { return <div className={map.active} />; }\n([...alias[\"active\"]] = source);",
    "const map = { active: \"flex\" };\nfunction Row() { return <div className={map.active} />; }\nfor ({ tone: map.active } of source) {}",
    "const map = { active: \"flex\" };\nconst alias = map;\nfunction Row() { return <div className={map.active} />; }\nfor (alias[\"active\"] in source) {}",
    // Duplicate object keys use their final reachable value for an unknown
    // lookup; a dynamic final write keeps the map clean.
    "const map = { active: \"flex\", active: getTone() };\nfunction Row({ key }) { return <div className={map[key]} />; }",
    // Only the final sequence expression supplies class output.
    "const tone = \"flex\";\nfunction Row({ dynamic }) { return <div className={(tone, dynamic)} />; }",
  ],
  invalid: [
    // Scope resolution is independent of traversal order.
    {
      code: "export function Row({ status }) { return <div className={map[status]} />; }\nconst map = { active: \"flex\" };",
      errors: detachedAggregate("map"),
    },
    {
      code: "const classes = <const>\"flex\";\ncn(classes);",
      extension: "ts",
      errors: detached("classes"),
    },
    {
      code: "const map = <const>{ active: \"flex\" };\ncn(map.active);",
      extension: "ts",
      errors: detachedAggregate("map"),
    },
    // A final static duplicate property is the only reachable value.
    {
      code: "const map = { active: getTone(), active: \"flex\" };\nfunction Row({ key }) { return <div className={map[key]} />; }",
      errors: detachedAggregate("map"),
    },
    // The final sequence expression remains class-producing.
    {
      code: "const tone = \"flex\";\nfunction Row({ dynamic }) { return <div className={(dynamic, tone)} />; }",
      errors: detached("tone"),
    },
    // Computed pattern keys and default-value right sides are reads, not write
    // targets, so they must not invalidate an otherwise static aggregate.
    {
      code: "const map = { active: \"flex\" };\n({ [map.active]: target } = source);\nfunction Row() { return <div className={map.active} />; }",
      errors: detachedAggregate("map"),
    },
    {
      code: "const map = { active: \"flex\" };\n({ tone = map.active } = source);\nfunction Row() { return <div className={map.active} />; }",
      errors: detachedAggregate("map"),
    },
  ],
});

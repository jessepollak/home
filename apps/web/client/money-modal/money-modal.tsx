...
function FooterButtons({ primaryLabel, onPrimary, primaryDisabled = false, primaryType = "button", primaryAutoFocus = false, secondaryLabel, onSecondary, secondaryDisabled = false, action, actionExpired = false, submitting = false }: MoneyModalFooterProps & { action?: PreparedMoneyAction; actionExpired?: boolean; submitting?: boolean }) {
  ...
  return (
    <DrawerFooter>
      <Button size="lg" className="h-11" type={primaryType} autoFocus={primaryAutoFocus} disabled={primaryDisabled} loading={submitting} onClick={onPrimary} {...(active ? { [MONEY_ACTION_ID_ATTRIBUTE]: action.id } : {})}>{primaryLabel}</Button>
      {secondaryLabel && onSecondary ? <Button size="lg" variant="ghost" className="h-11" disabled={secondaryDisabled || submitting} onClick={onSecondary}>{secondaryLabel}</Button> : null}
    </DrawerFooter>
  );
}

...
function RevealProbe({
  ...
  scope: string | null;
  models: BalanceRowModel[];
  resetSignal?: number;
}) {
  const reveal = useBalancesRevealWindow(scope, models, resetSignal);
  return (
    <button type="button" onClick={reveal.extend}>
      {reveal.count}
    </button>
  );
}
...
describe("useBalancesRevealWindow", () => {
  test("preserves extension through volatile rows and resets only for scope or signal", () => {
    const view = render(<RevealProbe scope="owner-a" models={rows(25)} />);
    ...
    view.rerender(<RevealProbe scope="owner-a" models={refreshed} />);
    expect(count()).toBe("20");
    view.rerender(<RevealProbe scope="owner-a" models={refreshed.slice(0, 8)} />);
    expect(count()).toBe("8");
    view.rerender(<RevealProbe scope="owner-a" models={refreshed} />);
    expect(count()).toBe("20");
    view.rerender(<RevealProbe scope={null} models={refreshed} />);
    expect(count()).toBe("20");

    view.rerender(<RevealProbe scope="owner-b" models={refreshed} />);
    expect(count()).toBe("10");
    fireEvent.click(view.getByRole("button"));
    expect(count()).toBe("20");
    view.rerender(<RevealProbe scope="owner-b" models={refreshed} resetSignal={1} />);
    expect(count()).toBe("10");
  });
});

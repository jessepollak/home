  ...
  return (
    <div className={modal.body}>
      <div className={styles.methods}>
        ...
        {regionId === "ID" ? (
          <button className={styles.method} type="button" onClick={onSelectIdrx}>
            <CurrencyMark currency="IDR" symbol="Rp" />
            <span className={styles.methodCopy}>
              <span className={styles.methodTitle}>Buy IDRX with rupiah</span>
              <span className={styles.methodHint}>
                Indonesia bank virtual account or QRIS
              </span>
            </span>
            <span className={styles.methodChevron} aria-hidden="true">›</span>
          </button>
      ...
      <div className={`${modal.body} ${styles.idrx}`}>
        ...
        {result?.presentation === "hosted" ? (
          <button
            ...
            type="button"
            onClick={() => onOpenCheckout(result.url)}
          >
            Open QRIS checkout
          </button>
        ) : null}
        <p className={styles.verificationCopy}>
          A provider request or payment is not proof of funding. Home verifies IDRX only by
          refreshing both the smart-account balance and activity.
        </p>
        ...
        {reconcileRequested ? (
          <p className={styles.reconcileStatus} role="status">
            Balance and activity refresh requested. Funding stays pending until IDRX appears
            in those reads.
          </p>
        ) : null}
      </div>
    ...
    >
      ...
      <fieldset className={styles.railGroup}>
        ...
        <label>
          <input
            type="radio"
            checked={rail === "bank-va"}
            onChange={() => setRail("bank-va")}
          />
      ...
      {rail === "bank-va" ? (
        <label className={styles.field}>
          ...
          <select
            value={channel}
            onChange={(event) => setChannel(event.target.value as IdrxVaChannel)}
          >

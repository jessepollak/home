...
export function CashOverviewExploration({ snapshot, balanceStatus = "ready", metadata, vaultStatus = "ready", nowMs, now = Date.now, rateLabel = null, onOpenSavings, onAddMoney, onRetryBalances }: CashOverviewExplorationProps) {
  ...
  return <div className="space-y-4">
    <Card variant="flush" aria-label={failed ? "Balance unavailable" : "Cash balance"} aria-busy={loading || undefined}>
      <CardContent inset="hero">
        <div className="flex flex-col gap-1">
          ...
          {loading ? <div data-shimmer="hero"><Skeleton className="h-10 w-48" /><span className="sr-only">Updating…</span></div> : <>
            ...
            {!failed && (empty ? bestRate !== null : Boolean(rateLabel)) ? <p className={!empty && earningApy ? "text-sm text-market-gain" : "text-sm text-muted-foreground"} data-cash-rate>{empty ? `Earn up to ${formatPresentationPercentage(bestRate!)} APY` : rateLabel}</p> : null}
...
export function SavingsDetailExploration({ snapshot, balanceStatus = "ready", metadata, vaultStatus = "ready", nowMs, now = Date.now, onDepositVault, onWithdrawVault, onRetryVaults, onRetryBalances }: SavingsDetailExplorationProps) {
  ...
  return <div className="space-y-4" onKeyDown={(event) => {
    ...
    {!balanceFailed && vaultStatus !== "failed" && other.length ? <section aria-labelledby="more-savings-heading"><Card><CardHeader><HomeSectionHeading id="more-savings-heading">More ways to save</HomeSectionHeading></CardHeader><CardContent inset="list"><ul className="list-none p-0">{other.map((candidate) => {

...
function CashStorySurface({ snapshot, balanceStatus: initialBalanceStatus = "ready", vaultStatus = "ready", homeParity = false, pendingExecution = false, ticking = false, snapshotToggle = false, reducedMotion = false, initialView = "cash" }: SurfaceProps) {
  ...
  if (mode !== null && availableBaseUnits === null && !dispatched) setMode(null);
    ...
    <PresentationRegionProvider regionId="US">
      <SavingsDialogFixtureProvider value={{ motion: reducedMotion ? "reduced" : "system" }}>
        ...
        <div ref={surface} className="min-h-svh bg-muted/50">
          ...
          {targetCandidate ? <SavingsMoneyDialog open={mode !== null} mode={mode ?? lastMode} session={session} candidate={targetCandidate} availableLabel={availableBaseUnits ? `${formatUsdStablecoinAmount(availableBaseUnits)} available` : undefined} availableBaseUnits={availableBaseUnits} availableStale={availableBaseUnits === null && dispatched} prepareMoneyAction={prepareMoneyAction} executeMoneyAction={executeMoneyAction} onClose={() => { setDispatched(false); setMode(null); }} onClosed={restoreDialogFocus} /> : null}
        </div>
        </MoneyMotionProvider>

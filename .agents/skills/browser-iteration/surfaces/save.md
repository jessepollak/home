...
- **Notes**: On an unfunded Save landing the `Start saving` FeatureIntro has a single `Get started` that opens Deposit (not a separate bottom `Deposit`); the deep-linked deposit still works. The smoke path dismisses it with Close and browser Back; it also opens Deposit and Withdraw from their buttons, closes via Close or Escape, and asserts focus returns to the exact opener.

...
- **Expect**: scroll container `[data-app-main-authenticated]` (shell-panels.tsx); balance rows `[data-balance-list] [data-kind="balance"]` (balances.pw.ts); a muted `[data-total-status]` line above the list shows `Partial` for a partial total (even when no row reads `Unavailable`), `Balance unavailable` for a failed read, or the choose-a-country prompt; reveal window grows after scroll (`BALANCES_BATCH_SIZE = 10`, client/home/balances-panel.tsx); `Show small balances` switch lives in account settings, not this page (mobile-geometry.pw.ts touch test).

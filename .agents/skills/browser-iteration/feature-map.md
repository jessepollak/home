...
- **Reach (live)**:
  ...
  8. `click "Send $1.00"`
  9. `expect "Sent"`
- **Notes**: The dialog is labelled by `send-title`. Live mode replaces `<recipient>` with the effective recipient: `--recipient` accepts a bare 40-hex `0x` address or exactly `jesse.base.eth` (mapped to the pinned `0x2211d1d0020daea8039e46cf1367962070d77da9`) and defaults to that pinned recipient when omitted; every other value refuses before browser launch. The product does not resolve recipient names yet (#720). Before the gated `Send $1.00` confirm click the verifier requires the review `To` row to equal that recipient (case-insensitive, full address or the shared `formatAddress` truncation) and stops with `recipientMismatch` otherwise.

...
The reference examples from [issue #654](https://github.com/jessepollak/home/issues/654) are the current worked example: three paired Home + Save directions — overview-to-vault workspace (recommended), expandable statement, and tabbed workspace — at story IDs `reference-directions-home` / `reference-directions-save` (`--overview`, `--statement`, `--workspace`, `--overview-desktop`), plus the retained `reference-home` ledger/tiles comparison and the connected `reference-journey`, and the map in the [design system](../../../docs/design-system.md#reference-journey-proposal-654). They are **direction examples, explicitly unapproved until Jesse selects one**; production adoption is a separate dependent issue. Reading them never authorizes copying the composition into production, bypassing browser validation, or treating a fixture journey as routing or money proof.

When Jesse explicitly authorizes direction exploration, this phase is not bound by the smallest-cohesive-improvement default in step 4 above: building several parallel, proposal-only compositions and their stories beside the production surfaces is the point of the exercise. What stays binding is the boundary — no production route, shell, token, provider, flow, dependency, or second UI system changes, and nothing is adopted before selection.

...
4. **Prefer the smallest cohesive improvement.** Improve the existing pattern rather than replacing it. A dependency, second component system, or demo route needs a separate approved decision. Keep Tailwind utilities inline at the product use site: do not detach them into class-string constants. Reusable presentation belongs in owned `components/ui` variants, and Oxlint (`home/no-detached-class-constants`) rejects identifiers in `className`/`cn()` that resolve to local static class strings (see [design-system rules](../../../docs/design-system.md)).

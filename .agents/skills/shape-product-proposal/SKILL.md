...
For a feature whose purpose is to move money, apply the verification ladder in `docs/operating-manual.md`. A provisioned agent may use the bot account only under the browser-iteration skill's live-confirm rules. The dedicated bot account's small balance bounds money risk. If a required live rung cannot be performed, state `Real money: not tested` and name the exact reason rather than calling the live path proven.

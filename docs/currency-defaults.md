...
| IDR · Indonesia | IDRX | Verification pending | Issuer confirms Base `0x18Bc5bcC660cf2B9cE3cd51a404aFe1a0cBD3C22` (checksum; lowercase in the candidates JSON) and **2** decimals — preserve token precision; do not infer from IDR fiat digits. Issuer mint API is documented (bank VA / hosted checkout; QRIS on checkout). Funding route is **not** smoke-tested; keep `enabled: false`. [IDRX contracts](https://docs.idrx.co/introduction/supported-chain-and-contract-address), [onramp research](idrx-onramp.md) |

...
- Run 13 (2026-09-23) answers five Codex code-parity findings. `Card` gains `variant=flush, inset=hero` (`301:5912`), and every `TotalBalanceCard` in `Home — final` and `Home states` uses it, as `HomeTotalBalance` does; the frames render unchanged. `TradeActions` draws Sell as `secondary`, as code does. `TransactionAmount` is now marked design-only, and the `ShimmerRow` `Show media` / `Show context` booleans are marked proposed; each has a follow-up. The publish count now includes run 12's `SignedBalanceBar` set. Nothing was published.
...
- **One block for grouped units (revision 3, componentized in revision 4).** Every group of rows or steps is one `Card` instance (`269:5270`) whose `CardContent` slot holds the rows. `variant=flush, inset=list` is the standard block: `color/card` fill, 1px `color/border` inside, `radius/xl` and a `space/1` (4px) inset. Rows inside keep Item's 12px side padding and drop their own dividers, so content sits 16px from the block edge as in code. `variant=default, inset=list` adds a `CardHeader` with the section heading (Home's Your money card). `variant=flush, inset=hero` (run 13) is only Home's total-balance card, matching `HomeTotalBalance`'s `Card variant="flush"` + `CardContent inset="hero"`: 16px padding and 8px between items at 390px (code widens the padding to 20px at `sm` and up). `variant=default, inset=default` is the padded card; StatusSteps override its padding to `space/3` / `space/4` (12/16), so step icons land on the same 16px line as receipt labels. It is used for receipts and steps, the review summary, the picker's held assets, the failed result's "Still in" row, rate cards, Your position and Details.
...
| Card | yes | `269:5270` set (run 11): `variant=flush, inset=list` (standard list block), `variant=default, inset=list` (CardHeader + list), `variant=default, inset=default` and `variant=flush, inset=hero` (`301:5912`, run 13: the Home total-balance card, as in `client/home/home-overview.tsx`), each with a native `CardContent` slot. `12:28` is kept as `inset=legacy` (the #683 stretched background layer, used only by Archive since run 12 rebuilt `Home states`) and keeps its mapping; new work never places it | yes | none (div) | `components/ui/card.tsx` | P0 |
...
| ShimmerRow | yes (`ShimmerRows`) | `274:6015` (run 11): one Item size=sm row with the Shimmer mark and two Skeleton lines, as `ShimmerRows` renders. The `Show media` and `Show context` booleans (label/value loading rows on the Finance patterns board) are **proposed**: `ShimmerRows` takes only `count` and always renders the mark and both lines (follow-up below) | yes | Item + Skeleton | `client/home/panel-shared.tsx` | P0 |
| TradeActions | yes | `274:5577` (run 11): Buy (`default`) + Sell (`secondary`, as code; run 13), touch, 34px home-indicator inset; pinned to the bottom of asset detail. Code ships only the unavailable state today, both buttons disabled over "Swaps aren't available right now.", which the Finance patterns board draws with `state=disabled` | yes | Button | `client/trading/trade-actions.tsx` (`layout="sticky"`) | P1 |
| TransactionAmount | **no** (`TransactionDetailsModal` renders the title and a `<dl>`; Amount and Status are ordinary rows from `presentActivityTransferDetails`) | `282:5882` tone=default\|success (run 11): signed amount + exposed status Badge. **Design-only** since run 13 (adoption follow-up below) | yes | Badge | unassigned (follow-up); screen owner `components/transaction-details.tsx` | P0 |
...
- `ShimmerRow` `Show media` and `Show context`. Map only the default with both on; `ShimmerRows` takes only `count`.
...
1. **Check the dialog lists the new items.**
   - 41 new components or sets: Icon; 21 new primitives, Badge to RadioGroup in the matrix plus `InputOTP` and `InputOTPSlot`; 12 new finance components, MoneyNumpad to PriceChart, counting `MoneyNumpadKey` and `MoneyConfirmRow` as their own sets; run 11's `Card` set, `TradeActions`, `SystemKeyboard`, `ShimmerRow` and `TransactionAmount`; and run 12's `Popover` (`292:5883`) and `SignedBalanceBar` set (`293:5935`). The two new sets that absorb an existing component (`Card` and `SignedBalanceBar`) are counted once here; the absorbed `12:28` and `91:927` appear under the changes below.
...
2. **Check the changes to existing items.**
   ...
   - Run 13 changes: the `Card` set gains `variant=flush, inset=hero` (`301:5912`), and the seven `TotalBalanceCard` instances in `Home — final` and `Home states` switch to it with no visual change. `TradeActions` Sell becomes `variant=secondary`. `TransactionAmount` becomes `DESIGN-ONLY`, and `ShimmerRow`'s description marks its booleans `PROPOSED`.
...
- **feat(activity): TransactionAmount header** (design-only `282:5882`). Lead `TransactionDetailsModal` with the signed amount and a status `Badge`, and drop the Amount and Status rows that `presentActivityTransferDetails` emits today. Money in uses `market-gain`. Transaction detail board. P1.
- **feat(ui): ShimmerRows row shape.** Give `ShimmerRows` props for label/value loading rows (no mark, one line), matching `ShimmerRow` `Show media` / `Show context` off. Until then, those boards are proposed and map to nothing. P2.
...
- Boards label proposed changes to current behaviour in their treatment text: the submit loading state, the 44px retry action and the 44px AlertAction. The `MoneyPrimaryAmount` error state, the `PriceChart` gain/loss tone and the `ShimmerRow` label/value rows are proposed too, and `TransactionAmount` is design-only; their component descriptions say so. Every other state mirrors current code.

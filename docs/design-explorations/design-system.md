...
- **Names match code.** A Figma component uses the code export name (`Button`, `ToggleGroup`, `MoneyNumpad`). Variant properties use the cva prop names, and their values are a subset of the cva values.
  ...
  - `MoneyNumpad` and `MoneyNumpadKey` are **deprecated** (revision 2). They are on the Archive page pending deletion, without a new Code Connect mapping; `amount.tsx` still renders the numpad until the native-input follow-up. New boards never place them.
...
| Card | yes | `269:5270` set: `variant=flush, inset=list` (standard list block), `variant=flush, inset=hero` (`301:5912`, HomeTotalBalance: 16px padding, 8px slot gap) used by every TotalBalanceCard on Home — final and Home states, `variant=default, inset=list` (CardHeader + list), and `variant=default, inset=default`, each with a native `CardContent` slot. `12:28` is `inset=legacy`, used only by the Archive pending deletion (20 instances); its variant mapping stays until deletion, while the set-level mapping is on `269:5270` | yes | none (div) | `components/ui/card.tsx` | P0 |
...
| TransactionAmount | **no** (design-only); `TransactionDetailsModal` renders a modal title and a `<dl>` with ordinary Amount and Status rows from `presentActivityTransferDetails` | `282:5882` tone=default\|success: proposed signed amount (money in `market-gain`) + status Badge header; see the [adoption follow-up](#follow-up-code-issues) | yes | Badge | `components/transaction-details.tsx` | P0 |
...
| MoneyNumpad (+ Figma sub-part MoneyNumpadKey) | yes (keys are inline `Button variant="ghost" className="h-14"`) | `166:1739`, `166:1738`: **deprecated**, moved to the Archive page pending deletion; boards use the native keyboard | no (to be removed) | Button ghost h-14 | `client/money-modal/amount.tsx` | — |
...
2. **Check the changes to existing items.**
   ...
   - `Card` set `269:5270` has the #841 HomeTotalBalance description; `12:28` is the legacy variant used only by the Archive pending deletion.
   ...
   - Run 11 changes (revision 4):
     - `FinanceRow` is restructured in all five variants (body wrapper, Icon chevron, in-slot primary Retry glyph, text styles, 12px padding). Every instance, including `Home — final`, Archive and Home states, re-renders with 58px rows; #841 subsequently centred lone values in Figma (proposed).
   ...
   - Run 12 changes (revision 5):
     ...
     - `Home states` (`190:2821`) is rebuilt on `Card`, `SignedBalanceBar`, `ShimmerRow`, `Skeleton`, `Empty`, `Button` and `Popover` instances; #841 subsequently moved it onto the Home page beside Screens. `Card inset=legacy` is now used only by Archive.
...
- **feat(activity): TransactionAmount header.** Put a signed amount (money in `market-gain`) and status Badge at the top of `TransactionDetailsModal`, replacing the Amount and Status rows; Figma `282:5882` is design-only until adoption. P1.
...
- **feat(money-modal): native numeric amount input.** `MoneyPrimaryAmount` becomes an auto-focused `inputMode="decimal"` input with a caret. Figma's `MoneyNumpad` and `MoneyNumpadKey` await deletion on the Archive page. Remove `MoneyNumpad`, `applyNumpadKey` and the key haptic in the same PR (knip). Add a tap-to-focus fallback for iOS, and move chips above the CTA. Verify with `agent-browser` and a device. Includes the over-balance state (the old "amount invalid state" item). P0.
...
- **IDs are contracts.** Swap instances in place and never delete or recreate a mapped component. After a named version is saved, history frames and deprecated components with no live instance outside the deletion set and no Code Connect mapping may be removed. The Archive page and legacy Card `12:28` await that version or Jesse's approval of the automatic version before deletion.
...
- Archive (pending deletion, #841) holds `Home — funded`, v1, v2, explorations and deprecated MoneyNumpad parts until the version-history precondition is met; none were deleted in #841.

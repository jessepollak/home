...
| --- | --- | --- | --- |
| MCP (`https://api.mobbin.com/mcp`) | OAuth | Pro, Team, Enterprise | The factory's only route, through the agent harness's MCP gateway |
...
- Jesse holds the paid plan. Before the subscription, the first MCP search on 2026-09-23 returned `Mobbin MCP requires a paid plan`; that error means the authorized account is not the paid one.
...
Mobbin is configured in the agent harness's MCP gateway, as server `mobbin`, on the machines that run factory work. The gateway holds the OAuth session; nothing Mobbin-specific lives in harness configuration or the repository. There is no Mobbin token file. The smoke below proves access on the host that runs it.

A factory worker calls the gateway's `mobbin.<tool>` tool with that tool's arguments. Discover the tools and schemas through the gateway's own search, list, and describe tools for the `mobbin` server.

### Smoke

The smoke uses no secret and costs no credits. Call `search_flows` with:

```json
{ "platform": "ios", "query": "borrow against crypto collateral", "limit": 3 }
```

A pass returns a `flows` array whose entries have `app_name`, `name`, and a `https://mobbin.com/flows/<id>` `mobbin_url`.

Evidence from a factory run on a provisioned runner on 2026-09-23:
...
- **One seat, one person.** Account sharing is prohibited (section 4.1). Home's reading: the factory uses Jesse's seat only as Jesse's agents on Jesse's hosts through the configured gateway session, and no other person uses it.

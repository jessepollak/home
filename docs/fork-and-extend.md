...
- `bun check` is the same gate CI runs (`bun install --frozen-lockfile` then `bun check`) if you send a focused PR. User-visible UI / core-flow PRs should include screenshots ([UI PR previews](pr-previews/README.md)).

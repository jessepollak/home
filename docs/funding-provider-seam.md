...
A provider-rejected or dispatch-ambiguous customer setup is terminal in the customer flow and is never retried automatically. An operator must reconcile the durable row with the provider and explicitly restart setup through a separately reviewed recovery process. No such reset or `verified` promotion path is included here; `verified` remains unreachable until observed provider status or webhook evidence is implemented and reviewed.

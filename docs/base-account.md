...
During a Base Account attempt the sign-in sheet stays open: connection, signing, and verification progress appears once, inside the Base Account button, while the conflicting email controls are disabled. The sheet's Close control is the single cancellation path; it cancels the pending attempt, closes the sheet, and restores focus. There is no separate handoff overlay or modal.

...
On restore, Home reads the closed `home:account-provider` tab hint, the identity-free cross-tab `home:cdp-restore` marker, and the readable `home-cdp-live` cookie. Only `cdp-embedded`, `pending:cdp-embedded`, the durable marker, or an exact lowercase 48-hex `home-cdp-live` nonce activates CDP and makes Home wait for its initialization. A Base Account hint or no hint never mounts `CDPHooksProvider`; the native restore can expose its verified identity—or settle signed out—without CDP config, MFA, or refresh requests. CDP-only deployments skip the native `/api/session` restore entirely. The root layout seeds every valid Home session cookie with the same check `/api/session` applies, so the native restore probes that endpoint on mount only for a Home-session seed. With a CDP render-hint seed or no seed—a CDP restore or a clean anonymous visit—it settles signed out without the Bearer-less probe, which could only answer 401. This holds for the composite provider and for the native-only bridge used when no CDP project is configured. A later Base sign-in or explicit retry still verifies with the server. Selecting email sign-in activates the one document-root CDP island on demand. Concurrent callers share activation. Initialization is bounded at 10 seconds; a failed or timed-out island is unmounted, and the next attempt mounts one clean replacement provider. Home stores no CDP access or refresh token; token ownership and refresh scheduling remain inside the SDK.

On a reload, the root layout may also pass a provisional render seed from a valid HMAC-signed `home-session` or complete CDP hint pair. The seed selects only that session's owner-scoped device cache while SDK/native restore is still pending; status remains `restoring`, the SDK remains uninitialized, and validation, provider activation, actions, and balance network reads receive no authority from it. Once the live SDK restores the matching owner and provisional smart account, only the owner-fenced balances read may overlap server verification; other authenticated transport stays gated. The seed is consumed once. The first live SDK settlement retains the cache only when its provider-native owner matches (`nativeOwnerKey` for Home-native sessions; CDP `currentUser.userId`, equal to the session subject, for hints). A mismatch or signed-out settlement clears the private boundary. Sign-in, sign-out, verification loss, and every later boundary advance permanently discard the seed, so a persistent layout prop cannot reassert private state after navigation.
...
The session subject derives from the verified, lowercased address. The browser's connected address is not session authority: it must be present in the signed message, in the verify body, and in the successful server verification. Verification resolves smart-account signatures through ERC-1271 and ERC-6492 in viem and fails closed if the signature cannot be verified. The signed challenge is the only nonce state; a verified sign-in resolves its customer, credential, and known wallet after the response without blocking sign-in. Only CDP-verified email sign-in stores an email on its credential; Base Account and other sign-in methods do not supply one. A version-1, expired, wrong-TTL, origin-mismatched, or tampered challenge cookie is rejected.

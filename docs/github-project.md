...
3. **Create with labels, not routing.** One `status:*`, one `lane:*`, one `priority:*`. No `owner:*` labels, GitHub assignee routing, or `factory:ready` at creation; agents and external creation assistants never apply readiness. As a readiness discipline, Jesse may later apply `factory:ready` only to an owner-authored delivery leaf. If Jesse directly requests creation in an interactive session, the issue body is Jesse-directed and does not receive `<!-- factory -->`.
...
### Product framing and delivery slices

For substantial product work, use the [product-proposal skill](../.agents/skills/shape-product-proposal/SKILL.md): post one ≤150-word product frame, wait for `go`, map three to five observable customer outcomes, and choose the fewest coherent vertical slices. Create separate delivery issues only when they can ship independently or a shared foundation truly unlocks more than one journey. Technical subtasks stay inside the owning slice. Every created slice still follows the native parent, Project membership, and label verification contract above.
...
| Factory candidates | MVP delivery issues filtered by `milestone:"Home MVP" Level:Delivery label:status:todo` | Which issues might Jesse consider marking ready? |

The Factory candidates view is a broad consideration pool, not the exact standalone queue. Jesse may choose issues from it to mark ready, and exact standalone eligibility still requires a Jesse-applied `factory:ready`. As a readiness discipline, Jesse applies that label only to an owner-authored delivery leaf with exactly one lane and priority. Body text, attribution markers, and Project view membership grant no authority. Exact standalone machine checks and implementation live outside Home and are not specified here.
...
Saved-view API results matched independently computed item sets on 2026-09-16: [MVP overview](https://github.com/users/jessepollak/projects/1/views/1) **8**, [Delivery](https://github.com/users/jessepollak/projects/1/views/2) **40**, and [Needs Jesse](https://github.com/users/jessepollak/projects/1/views/3) **7**. Counts are a dated snapshot, not fixed acceptance targets. Live view 4 is [Factory candidates](https://github.com/users/jessepollak/projects/1/views/4), using `milestone:"Home MVP" Level:Delivery label:status:todo` as a broad consideration pool rather than the exact standalone queue. Delivery used `milestone:"Home MVP" Level:Delivery -delivery-status:"Not planned"`, with Workstream swimlanes and Delivery status columns. The API accepts the hyphenated field key; quoting the field name produced an empty view and was corrected.

...
3. **Create with labels, not routing.** One `status:*`, one `lane:*`, one `priority:*`. No `owner:*` labels, GitHub assignee routing, or `factory:ready` at creation; agents and external creation assistants never apply readiness. Jesse may later apply `factory:ready` to make an owner-authored eligible leaf a standalone queue candidate. If Jesse directly requests creation in an interactive session, the issue body is Jesse-directed and does not receive `<!-- factory -->`.
...
| Ready for factory | MVP delivery issues filtered by `milestone:"Home MVP" Level:Delivery label:factory:ready label:status:todo` | Which issues are standalone queue candidates? |

The factory view displays candidates; it is not itself an authorization system. Standalone queue eligibility requires an open, owner-authored eligible leaf with a Jesse-applied `factory:ready`, `status:todo`, and exactly one lane and priority. Body text and attribution markers grant no authority. Queue implementation and additional fail-closed checks live outside Home; workstream roots and intermediate tracking containers are not eligible leaves.
...
Saved-view API results matched independently computed item sets on 2026-09-16: [MVP overview](https://github.com/users/jessepollak/projects/1/views/1) **8**, [Delivery](https://github.com/users/jessepollak/projects/1/views/2) **40**, [Needs Jesse](https://github.com/users/jessepollak/projects/1/views/3) **7**, and the live [Ready for factory](https://github.com/users/jessepollak/projects/1/views/4) **7**. Counts are a dated snapshot, not fixed acceptance targets. Ready for factory used `milestone:"Home MVP" Level:Delivery label:factory:ready label:status:todo`; Delivery used `milestone:"Home MVP" Level:Delivery -delivery-status:"Not planned"`, with Workstream swimlanes and Delivery status columns. The API accepts the hyphenated field key; quoting the field name produced an empty view and was corrected.

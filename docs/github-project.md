...
3. **Create with labels, not routing.** One `status:*`, one `lane:*`, one `priority:*`. No `owner:*` labels and no GitHub assignee routing. An operator-invoked factory run starts only from `status:todo`; no readiness label or body marker grants authority. If Jesse directly requests creation in an interactive session, the issue body is Jesse-directed and does not receive `<!-- factory -->`.
...
| Ready for factory (live name) | MVP delivery issues filtered by `milestone:"Home MVP" Level:Delivery label:factory:ready label:status:todo` | Which issues were candidates under the previous runner policy? |
...
Saved-view API results matched independently computed item sets on 2026-09-16: [MVP overview](https://github.com/users/jessepollak/projects/1/views/1) **8**, [Delivery](https://github.com/users/jessepollak/projects/1/views/2) **40**, [Needs Jesse](https://github.com/users/jessepollak/projects/1/views/3) **7**, and the live [Ready for factory](https://github.com/users/jessepollak/projects/1/views/4) **7**. Counts are a dated snapshot, not fixed acceptance targets. Ready for factory used `milestone:"Home MVP" Level:Delivery label:factory:ready label:status:todo`; Delivery used `milestone:"Home MVP" Level:Delivery -delivery-status:"Not planned"`, with Workstream swimlanes and Delivery status columns. The intended **Factory candidates** rename/filter has no live snapshot count yet and remains the operator update stated above. The API accepts the hyphenated field key; quoting the field name produced an empty view and was corrected.

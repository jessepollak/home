...
Status: **G1 CDP-first server, G2 deletion, G3 server observation, G3b production completeness, G3c latency/price observations, and G4 dust default shipped** (2026-09-13). Subsystem design under [architecture.md](architecture.md) (principles 2 and 5; the balances snapshot is an *observation*) and the current home for the locked inventory direction. Home enumerates the wallet through CDP, resolves against registry ∪ Codex 512 ∪ wallet metadata, reads the registry at one pinned block, prices resolved rows, and stores the last observation in Neon. Home's own actions and CDP webhooks invalidate that snapshot; the 120-second refresh is a backstop. Add another enumerator only after measured CDP limits justify it, and do not add a self-hosted indexer without a shipped requirement. Provider failure stays `unavailable`, a successful zero stays `0`, and no cache authorizes an action.
...
- **`totals`** has `cash` (cash-currency holdings plus savings vaults), `investments` (every other holding plus Borrow collateral), `borrow` (a positive magnitude of all debt), and `net = cash + investments − borrow`, with `net.negative` carrying the sign. Each component follows the gross-total rules: `complete` only when every contribution is known and priced, `partial` when a known nonzero sum is missing something, and `unavailable` otherwise. Partial Borrow coverage makes Investments and Borrow incomplete, so `net` can never be `complete` without a full Borrow read, and a gross total is never reported as a complete net. Regions without a quote currency report every total as `no-quote-currency`. Home renders `totals` since #789 (`presentBalances` in `shared/balances/present.ts`): the net total, the signed allocation bar and legend, and the Cash, Investments, and Borrow Cash rows, with Borrow's rate as `N% APR` from each owing market's `borrowAprWad` weighted by its debt `value` (or by debt normalized to token decimals when any debt is unpriced). A partial net renders dimmed with a `Partial` badge beside `Total balance` but raises no header status: a large wallet's long-tail inventory or unpriced holdings keep coverage partial indefinitely, so partial coverage alone is not an interruption (#700, #826, #389). Home's own header status (`homeBalancesStatus` in `client/home/home-status.tsx`) appears only when the balances read fails (`Balances are unavailable`, with a Retry icon button) and clears after a successful read; with no quote currency (`needsCountry`) the popover shows the `Choose a country in Account…` prompt and opens Account instead. The offline and interruption statuses below take precedence over it. An unavailable net renders `—`; a failed summary row shows a muted `—` without a chevron, and a known but partial row (for example Borrow with partial coverage) renders its value muted. The gross `total` stays in the contract but has no Home consumer.

When verified dashboard balance reads fail because of a network error or server 5xx, Home confirms after 5 foreground seconds before showing the interruption status across tabs; a confirmed interruption rechecks every 30 foreground seconds up to ten times, then offers manual Retry. Offline status appears after 2 continuous foreground seconds and changes to the interruption status on reconnect; a successful read clears either status after 3 foreground seconds. Cached values remain display-only.
...
Membership rules (today's, made explicit): cash rows (canonical USD + selected local) always render, including at zero and when `unavailable`; non-cash rows render only with an authoritative positive balance — an `unavailable` non-cash registry row is **hidden** and surfaces through `coverage.registry: "partial"` → total status label, never as a wall of error rows; vault shares are never rows (they count in `total` and appear in Save via `selectVaultPositions`). Grouping and ordering: **Cash** (selected local, canonical USD, other cash — authored order, never re-sorted by value) and **Investments** (every other non-vault holding with a positive balance, priced by value desc → unpriced by name → dust last). Each group shows a quote-currency subtotal of its priced rows (omitted, never 0, when nothing is priced). These rows render on the Balances panel (`/balances`, `/balances/cash|investments`). Since #789 the Home "Your money" card instead shows three summary rows from `totals` — Cash, Investments (`Across N assets`, counting visible non-dust holdings and Borrow collateral once per asset key), and Borrow Cash — and the hero shows the signed Borrow · Cash · Investments breakdown. Formatting stays in `shared/formatting`. Dust behavior is described in §9.
...
| Constant | Value | Why |
|---|---|---|
| Client `staleTime` | 15 s | refetch on focus/mount past this; bounded 3 s retries for stale snapshots, and 30 s rechecks only during a confirmed interruption |
...
| Price observation refresh | 60 s | older safe stored observations are served and refreshed silently in the background |
| Negative valuation retry | unavailable 60 s; missing/invalid/stale 15 min | suppresses repeated provider misses without deleting last-good values |

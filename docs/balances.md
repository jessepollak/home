...
- **Prices never per owner.** Stored token observations and FX are read concurrently and applied at response time. Missing token observations fetch Codex in the foreground; observations fetched more than 60 s ago are served when their source time remains within 24 h and refreshed in the background. FX remains in its global short-TTL cache. The route is `private, no-store`.
...
| Price observation refresh | 60 s | older stored observations are served and refreshed in the background |

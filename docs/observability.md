...
`kind=activity-read` (`code=ACTIVITY_READ`) includes `route`, `outcome`, `reason`, `source`, `durationMs`, `sourceDurationMs`, `sourceAttemptCount`, `pageCount`, `rowCount`, and `valuation: { priced, unknownToken, noRecentClose, quoteUnavailable, fxUnavailable }`. These are per-returned-page counts (bounded to 10,000); started, failed, cancelled, and rejected events carry zero valuation counts. No addresses, amounts, token contracts, or timestamps are logged.

For `kind=funding-order` and `kind=funding-webhook`, `provider` is only the funding manifest ID (for example `coinbase`, `idrx`, or `ripio`), never the account provider. Generic order-route failures omit it when the funding provider is unknown. Core lifecycle events use only `route`, closed `code`, `outcome`, funding `provider`, `region`, optional boolean `sandbox`, and bounded `durationMs`; they never carry `ownerHash` or order-level data.

Operator aggregate queries should filter `schema=home.observability.v2 AND kind=funding-order`, then count `code=ORDER_CREATED`, `code=ORDER_RECEIVED`, `code=ORDER_AMBIGUOUS`, `code=ORDER_AMBIGUOUS_RESOLVED`, or `code=ORDER_SENT_UNVERIFIED`, grouped by `provider`, `region`, and `sandbox`. Compare received/created and ambiguous/created rates over the same time window; `ORDER_AMBIGUOUS_RESOLVED` counts owners who cleared an ambiguous order after the recovery wait, never a provider-confirmed cancellation. Keep omitted `sandbox` as an explicit unknown bucket rather than assuming production. These are aggregate rates, not per-order traces. Exact order acceptance evidence remains in `funding_orders` and the provider runbooks, not observability logs.

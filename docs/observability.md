...
Operator aggregate queries should filter `schema=home.observability.v2 AND kind=funding-order`, then count `code=ORDER_CREATED`, `code=ORDER_RECEIVED`, `code=ORDER_AMBIGUOUS`, or `code=ORDER_SENT_UNVERIFIED`, grouped by `provider`, `region`, and `sandbox`. Compare received/created and ambiguous/created rates over the same time window; keep omitted `sandbox` as an explicit unknown bucket rather than assuming production. These are aggregate rates, not per-order traces. Exact order acceptance evidence remains in `funding_orders` and the provider runbooks, not observability logs.

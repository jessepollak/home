...
For surfaces whose live Reach fills `To`, currently only send, `--recipient` is optional. Omitted, it defaults to `jesse.base.eth` and its pinned address `0x2211d1d0020daea8039e46cf1367962070d77da9`; it accepts a bare 40-hex `0x` address or exactly `jesse.base.eth`, and refuses every other value before browser launch. The product does not resolve recipient names yet; that work is tracked in #720. Immediately before a gated confirm click on such a surface, the verifier re-reads the review and requires a `To` row whose rendered value equals the effective recipient, case-insensitively, as the full address or the shared `formatAddress` truncation. A missing, empty, or different row stops before the click, writes `recipientMismatch` and `reviewText` to `live.json`, and exits 1.

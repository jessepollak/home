...
```sh
bun run --cwd apps/web verify <surface-id> --base-url http://localhost:3200 --out /tmp/home-verify
```

The CLI shells out to the repository-pinned `agent-browser`; it never prints cookies, browser state, or environment values. For an agent-driven run use `verify start <surface-id> [--live --base-url <url> --out <outside-repo-dir> ...]`, then `verify snapshot`, `verify click <@ref|name>`, `verify fill <label> <value>`, `verify press <key>`, or `verify goto </path>`, and end with `verify finish`. The agent uses fresh snapshots to navigate; the feature map's Reach steps are guidance. Each command is checked against the surface policy and appended to the session record. Use `--session <name>` on `start` and every later command to keep concurrent sessions separate under private `~/.home-verify/<name>.json` files; omitting it selects the default `active-session`. A failed ordinary click, fill, or app-path navigation returns an error and fresh snapshot without closing the session or recording a ledger run, so fix the step and continue. Policy refusals and any confirm-path failure close the session. Live `fill` accepts only mapped fields, live `press` refuses, and `goto` stays on the pinned app origin and outside `/api/`. Only `verify confirm` may activate a prepared money control. Agent-driven `verify confirm` authorizes the prepared action's kind against the selected surface (`kindForSurface`), not against a feature-map confirm label; the label still guides unattended Reach replay. The one-shot `verify <surface-id>` remains available for unattended replay and the studio canary, using the same guarded browser steps and confirm checks. Fixture runs stage deterministic signed-in, session, balance, action-list, recipient-name, recent-recipient, funding-list, and profile fixtures before navigation.

Fixture-mode `verify confirm` rehearses the same prepared-action checks for send using the verifier's pinned account and recipient; fixture-only prepare responses are captured in the private HAR with `--content all` and decoded from base64. Live HAR capture stays text-only. Fixture confirms have no live caps or ledger spend, and other fixture surfaces explicitly refuse with `no fixture prepared action for <surface>` until they have a prepared-action fixture.

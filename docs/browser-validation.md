...
Before `factory:review`, the factory runs Rung 0 before the first and after the last edit for every touched surface, Rung 1 for every mapped surface, Rung 2 for a touched money client flow, and Rung 3 for touched action, money-action, calldata, or confirm paths when the surface is armed. It does not wait for Jesse between rungs. `verify status` supplies arm and spend state; a disarmed surface, exhausted cap, or insufficient ceiling is reported as `Real money: not tested` with that exact bound. CI and GitHub Actions continue to refuse Live mode.

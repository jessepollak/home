...
Set the public `HOME_VERIFY_PRODUCTION_URL` and the studio environment described below, run `live-login`, and confirm `verify status` shows each confirm surface armed before the first weekly run. Evidence and command logs stay under `${HOME_VERIFY_CANARY_DIR:-~/.home-verify/canary}/<UTC timestamp>/`. The script creates an open issue titled `Verification canary` when missing, pins it whether it was just created or already existed unpinned, then posts the retained `summary.md`. A failed journey remains in the summary, exits non-zero, and any verifier incident separately disarms its surface and updates `verify: <surface> disarmed`.

For studio scheduling, copy `scripts/verify/com.jessepollak.home-verification-canary.plist` to `~/Library/LaunchAgents/`, replace every placeholder — `__HOME_REPOSITORY__`, `__HOME_PRODUCTION_HOST__`, `__HOME_DIRECTORY__`, and `__HOME_VERIFY_ACCOUNT_EMAIL__` — and load it with `launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.jessepollak.home-verification-canary.plist`. The template runs daily at 03:00 local time and deliberately contains no password or token. `canary.sh` prepends `$HOME/.bun/bin` to `PATH` because the launchd environment has no `bun`/`bunx` and the verifier shells out to `bunx agent-browser`. Provide the deployment access password either directly as `HOME_ACCESS_PASSWORD` in the launchd environment (from its secret store, without printing it) or as `HOME_ACCESS_PASSWORD_OP_REF`, a 1Password reference that `canary.sh` resolves with `op read` when `HOME_ACCESS_PASSWORD` is empty; keep Gmail refresh credentials only in the mode-`600` file. A system Chrome is enough for the browser step — `bunx agent-browser install` is unnecessary when `bunx agent-browser doctor` finds one. Run `weekly` manually once before relying on the schedule, then inspect the pinned issue and retained evidence.

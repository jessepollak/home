...
A live run that fills the cash-out payout handle (`Cash App handle` and `Re-enter handle`, whose feature-map Reach uses the `$alice` placeholder) substitutes `HOME_VERIFY_CASHOUT_HANDLE` for the placeholder (its Cash App canonical form, without the leading `$`, for `Re-enter handle`) so the run never registers a stranger's cashtag with Peer, and text evidence (`dom.txt`, `live.json`, `summary.md`) has the handle replaced by `<payout-handle>`; screenshots are not redacted, so keep the evidence directory private. When it is unset, the cash-out surface refuses before browser launch and before any fill, naming the variable; fixture Reach keeps `$alice`.

...
Set the public `HOME_VERIFY_PRODUCTION_URL` and the studio environment described below, run `live-login`, and confirm `verify status` shows each confirm surface and the current spend before the first weekly run. Evidence and command logs stay under `${HOME_VERIFY_CANARY_DIR:-~/.home-verify/canary}/<UTC timestamp>/`. The script creates an open issue titled `Verification canary` when missing, pins it whether it was just created or already existed unpinned, then posts the retained `summary.md`. A failed journey remains in the summary and exits non-zero, and any verifier incident is recorded on its run's ledger entry and reported by `verify status`.

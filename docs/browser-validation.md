...
2. **Does a regression need a permanent automated test?** Follow this order:
   ...
   The [test-weight CI gate](gates.md#browser-test-ladder-boundary) asks for a PR-body `Playwright-rung` when a new Playwright call is added and a `Test-weight` reason when a scoped fix adds more test than product lines.

...
4. In the PR body record `Verified: <surface> rung <n>` with safe evidence, or `Not verified: <surface> rung <n> — <reason>` naming the blocker. Keep the existing `## Verification` surface rows and evidence pointers. Update the feature map in the same PR if the live UI differs from it.

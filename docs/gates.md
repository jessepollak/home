...
- `home/no-silent-catch` rejects empty or unhandled catches in `app`, `components`, `config`, `shared`, and root `instrumentation*.ts` files. Recovery state, typed non-null results, explicit reporting, promise settlement, and cleanup remain valid handling. `client` and `server` are excluded pending #712, which tracks 80 findings across 48 files. The rule targets the state-and-cache failures represented by `a3efbae2`, `01042089`, and `14465e42`.

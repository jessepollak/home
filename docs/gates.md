...
`bun run --cwd apps/web verify start <surface-id>` opens an agent-driven fixture session; `verify snapshot`, guarded commands, and `verify finish` produce screenshot, browser snapshot, errors, marked action ids, and a `summary.md` bundle. Reach in the [feature map](../.agents/skills/browser-iteration/feature-map.md) is guidance, with Playwright as the sole committed automated browser regression layer. See [browser validation](browser-validation.md#surface-verify-cli) for setup, output, and cleanup.

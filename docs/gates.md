...
The **Test weight** CI step compares `origin/<BASE_REF>...HEAD` with the pull-request body and title. A newly added `test(` or `test.describe(` line in `apps/web/tests/browser/**/*.pw.ts` (including nested calls) requires a `Playwright-rung: <rung>` PR-body line, with a rung of `layout`, `scrolling`, `focus`, `history`, `persisted-state`, `media-query`, `hydration`, `dispatch`, or `journey`. A scoped `fix(...)` title whose added test lines (`*.test.ts`, `*.test.tsx`, `*.pw.ts`, `*.stories.tsx` under `apps/web`) exceed added plus deleted product source lines under `apps/web` requires `Test-weight: <reason>` in the PR body. Product source excludes test/story/verify files. No status-derivation or amount-parsing path exemption is encoded: there is no unambiguous path convention for those behaviors, so use the reason line when they trigger. The gate checks the declaration, not whether a proposed rung or reason is persuasive; review still owns that judgment.

The step is soft (`continue-on-error`) during calibration. A follow-up PR makes it hard after three consecutive applicable pull requests report no false-positive Playwright trigger or test/product count; until then, a failure is a required handoff finding but does not block CI. The six `bun run gates` invariants above are unchanged because this check reads PR metadata in CI rather than running in `bun check`.

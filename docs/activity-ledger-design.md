...
| Cash-out orders | `Cash out to <payout app>` from the provider progress platform label (`client/activity/activity-ledger-items.ts`) | `Cash out to bank` / `Cash out to Venmo` | Only Peer cash-outs recorded as Home actions appear; no cross-region order history. |

...
- **`typescript/switch-exhaustiveness-check` (`considerDefaultExhaustiveForUnions: true`).** Seven current switches (`server/actions/prepare.ts`, `client/account/cdp-wallet-provider-capabilities.ts` ×2, the since-deleted `client/actions/review.tsx`, the since-deleted action-detail presenter, `client/account/account-screen.tsx` ×2) all carried an intentional `default` fallback, so none was a defect. Enabling the rule with `considerDefaultExhaustiveForUnions` keeps a precise, non-noisy boundary: switches without a `default` over a union must still enumerate every case (proved by a canary fixture).

...
- `GET /api/market-prices/history` accepts the configured static asset IDs plus canonical lowercase `base:0x…` IDs for read-only dynamic Base assets. Other networks, malformed IDs, mixed-case aliases, and dynamic aliases of configured assets fail closed before a provider request. This resolver is not used to grant execution eligibility; trading keeps its separate static gates.
- History requests use fixed server-owned windows/resolutions, return only the requested canonical identity, and remain short-cache public reads. The history contract and chart explicitly label values as `USD`. The local-fiat current-price header is unchanged, and Home does not apply today’s FX rate to historical chart values or return labels.

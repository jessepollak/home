...
Actions use the thin model: one confirmed `actions` row, server-authored calldata, client dispatch through CDP or Base, and provider/chain-derived status. `DATABASE_URL` is required for actions; the database is disposable. See [Actions](actions.md).

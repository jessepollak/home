...
When a cash quantity is known but its fiat valuation is unpriced, Home shows the formatted native token amount with its token symbol—for example, `2,500.00 IDRX`, not an implied `Rp 2,500.00`. Failed reads remain `Unavailable`; unknown quantities remain unknown. This presentation fallback does not manufacture a quote, change valuation status, or add unpriced holdings to the aggregate.

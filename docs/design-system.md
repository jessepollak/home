...
- `ItemMedia variant="avatar"` owns the standard circular row media. `ItemTitle` accepts `tone` and `numeric`; `ItemDescription` accepts `lines={1 | 2 | false}`, defaults to two lines, and disables line clamping when set to `false`.

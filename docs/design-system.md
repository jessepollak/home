...
- `PayoutMark variant="cashapp" | "zelle" | "monzo" | "revolut" | "fallback" | "count"` owns the circular payout brand mark geometry and brand colors.
...
1. Style components and product surfaces with Tailwind utilities. ESLint bans hex/rgba, arbitrary-pixel, and raw palette classes; use semantic tokens instead. Keep utilities inline at the product use site: ESLint (`tailwind-policy/no-detached-class-constants`) follows identifiers in `className` and `cn()` to local static class-string constants, and local object/array lookups to fully static class maps, and rejects them. Reusable presentation belongs in owned component variants; dynamic composition (ternaries, templates, props, `cva()`) stays allowed.

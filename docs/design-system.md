...
A review board is a Storybook story that arranges live, isolated story canvases on one page; it is not part of the production application. Board manifests live in `apps/web/stories/review/boards/` and contain only story IDs, viewports, section labels and review metadata. Add a manifest there, validate it with `parseBoard`, and register a named story in `review-boards.stories.tsx`. Build Storybook to check story IDs against `index.json`; unavailable stories are labeled explicitly.

The board URL keeps Storybook `id` and `viewMode` plus `frame` (selected frame), `side` (`after` or `before`), `rev` (build revision), and `deployment` (build host). Desktop opens on a fitted pan/zoom canvas, with a collapsible section/frame outline and a selected-frame inspector; select a frame in the outline to fit it, then use Interact to enter its live iframe (Esc returns to navigation). Wheel pans and Ctrl/Command + wheel zooms at the pointer. `+`/`-` zoom, `0` resets, `1` fits the board, `2`/`F` fits the selected frame, and arrow keys pan. Mobile shows one live frame at a time with a grouped journey picker, Previous/Next actions, and a full-width interactive view. The runtime is design-lane code under `apps/web/stories/review/explorations/board/` and is imported only by review stories, never production bundles.

Open the board on the PR's commit-specific Storybook preview deployment, whose URL is unique to one build; the branch preview alias moves with later pushes. Review comments use the preview's Vercel Comments. On load the board writes the build's `rev` and `deployment` into its URL, so a comment's page URL records the revision it was made on, and each frame is covered by an element carrying `data-review-frame` and `data-review-story` for comments to anchor to. Opening that URL on a newer build shows a banner linking back to the reviewed deployment. To recover a comment's context, run `bun run --cwd apps/web review:context '<comment page URL>'` (`--json` for machine output). It prints the board, frame, story, viewport, notes, story source (after `build-storybook`), story and board links on the reviewed deployment, and whether the story or manifest changed between the reviewed revision and `HEAD`. Comments stay in Vercel. Record dispositions on the GitHub issue or PR.

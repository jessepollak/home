...
1. Style components and product surfaces with Tailwind utilities. Oxlint bans hex/rgba, arbitrary-pixel, and raw palette classes; use semantic tokens instead. Keep utilities inline at the product use site: the Home Oxlint rule `home/no-detached-class-constants` follows identifiers in `className` and `cn()` to local static class-string constants and rejects them. Reusable presentation belongs in owned component variants; dynamic composition (ternaries, templates, props, `cva()`) stays allowed.

...
Fail-closed stays: true provider failure → `unavailable`; successful zero stays ready `0`; never invent balances. A CDP omission is not contract authority for configured ERC-20s: the current Phase A path sequentially attempts each unresolved configured contract once through configured `BASE_RPC_URL` (maximum 20 deduped contracts inside one shared four-second stage), retains completed singles and the original incomplete/unavailable state across timeout or RPC failure, propagates caller cancellation, and never falls through to the public default. Session boundary unchanged.

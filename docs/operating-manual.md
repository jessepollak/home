...
**Factory execution contract.** The factory is pull-based and may start only from an issue carrying `factory:ready`. There are two fail-closed authorization routes. `legacy-human-body/v1` requires an unmarked open issue authored by the configured repository owner. `approved-factory-brief/v1` requires a marked child listed in an exact machine-readable, factory-marked parent proposal comment with exactly one current repository-owner +1. Non-owner reactions are ignored. Of multiple valid owner-approved comments matching the current exact child, the newest stable comment creation/ID order wins and that identity is retained. The marked route verifies repository, parent and child identities, native parent, exact title/body hash/labels, the child's mapped outcomes, unchanged comment, continued reaction, open state, and no conflicting PR. Edits or reaction removal revoke execution even if a ready label remains; existing marked issues are never retroactively approved.
...
Publication is non-authorizing and never adds `factory:ready`; it preserves unrelated labels, fails closed on conflicting routing labels, and activation tolerates unrelated labels that do not change authorization. After Jesse reacts, `bun run factory:brief activate <parent-number>` verifies the exact proposal and every selected child before mechanically adding ready labels, so one approval can activate all children. Only the deterministic supervisor reads proposal comments, reactions, native parent, and linked PR state. Each child receives only its exact retained body and mapped outcome subset—not mutable issue prose, unrelated comments, or other children's outcomes. The supervisor revalidates the same approval before the initial worker, every remediation, and final handoff, and checks again for a conflicting PR before creating this run's PR. Each run gets one worktree, an `agent/*` branch, one issue, and one normal open PR to `main`. Run `bun run factory:preflight` before work. GitHub credentials stay with the supervisor; child model processes receive none. The protected `main` ruleset has no automation bypass. No production, provider, database, funded, destructive, deployment, privileged-setting, or merge authority is granted. Incremental Vercel spend is capped at **$100/month above baseline**; the factory adds no Vercel service or credential.

### Manual single-issue runner

From an authenticated clone, run `bun run factory:run <issue>`. This is a manual one-issue command, not a queue, scheduler, or daemon. It fails closed unless one authorization route above verifies, the issue is open, carries `factory:ready`, exactly one `status:*` (`status:todo`), exactly one lane, exactly one priority, and has no open PR referencing it. An atomic host lock allows one active run; touch `$(git rev-parse --git-common-dir)/factory-stop` to stop at the next stage boundary, then remove that file before a later run. `bun run factory:run <issue> --dry-run` exercises separate bounded worker and reviewer child processes without a model call or GitHub mutation and writes no durable run evidence.

The bounded drain pilot recorded in [#548](https://github.com/jessepollak/home/issues/548) remains stopped until the issue-author and unmarked-body hardening in [#549](https://github.com/jessepollak/home/issues/549) is merged and deployed. Resumption does not widen the eligibility contract above.

The supervisor creates `agent/<issue>-factory-run`, runs preflight before model work, opens one normal PR, and launches fresh no-session worker and read-only reviewer processes with hard timeouts. Each child receives an invocation-owned temporary home/config directory, only its selected Pi model/provider configuration, no stored GitHub authentication, and no Git credential helper; the directory is removed when the child exits. A timeout terminates the child's process group on supported macOS/Linux hosts so descendants cannot retain its output pipes. A timeout, malformed verdict, or incomplete verdict fails review. Approved briefs also require exactly one concise `Met`, `Not met`, or `Unverified` assessment with evidence/reason for every required outcome. Missing, extra, or duplicate IDs fail closed; a pass with any non-Met assessment is invalid. `Not met` and `Unverified` are blocking and use the same mechanical maximum of two fix/review loops. Unresolved outcomes end `status:needs-jesse`, never passed. Legacy runs retain the prior review schema. Worker result JSON is not required for non-user-visible work; required browser assessment remains structured for user-visible lanes. Two fix/review loops are the mechanical maximum. Required CI must be green on the unchanged current PR head, and user-visible lanes must already contain the required current-head preview URL and media, before either record is promoted to `status:needs-jesse`; a pending, failed, errored, or timed-out gate leaves both records `status:working`. It never writes `main`, changes draft state, approves, merges, or auto-merges. Concise evidence is retained under the common Git directory's `factory-runs/`; it is run evidence, not a board. The command exits after its bounded run and does not watch for Jesse review comments. Comment pickup and fix/resume remain explicit future/manual flows; this command does not implement them.

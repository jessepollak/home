...
A design or library pass is complete as a design deliverable when its handoff maps every follow-up. Production adoption may remain outstanding; state that explicitly. Before handoff, reconcile every proposed component, state, token, and behavior against current code and open and closed issues and PRs.

Give each follow-up exactly one disposition: **Implemented** (landed PR and evidence); **Covered** (linked existing issue whose scope explicitly includes it); **Filed** (new issue with customer outcome, acceptance, design reference, dependencies, and verification); **Deferred** or **Rejected** (reason and decision owner). A partly delivered follow-up is Implemented only for its landed scope; each outstanding part gets its own row and disposition. If selection is pending, link the decision or adoption issue and name the dependency. File or reuse issues within Jesse's authorized scope. `factory` goes on an issue only when execution is authorized, including broad "queue all" authorization; Jesse applies it ([Factory runs](#factory-runs)). After Jesse authorizes filing or queueing, do not leave a follow-up at "Jesse decides which to file". Research or exploration alone is not implementation approval.

The design document carries one concise coverage table—follow-up, disposition, delivery link, state—and the PR body links it. Do not leave orphan prose TODOs; each design/code difference a follow-up leaves open gets its own row. Count implemented, covered, filed, deferred, and rejected items so the totals equal the table's rows. Report actual states (open, `factory:working`, `factory:review`, merged) read at edit time. A label is not running or landed code; a link establishes ownership, not shipped code. The independent reviewer checks coverage and design/code parity. Claim production adoption only from landed code, not a Figma publish, a design PR merge, removal of archived design nodes, or an issue closed for consolidation.

Adoption PRs update the gap matrix and Code Connect parity (`apps/web/figma-components.json`) and close only their delivered scope. One writer coordinates each shared component. Reuse existing issues and prefer few coherent vertical slices. Use the existing issue and PR workflow; add no board, manifest, scheduler, approval step, or reporting system. See [library pass coverage](design-explorations/design-system.md#follow-up-code-issues).
...
3. **Independent review.** Review is fresh, read-only, scoped to the complete current diff, and time-boxed. The writer cannot review its own change. A design or library handoff review also checks [design handoff coverage](#design-handoff-coverage) and design/code parity.
...
Product docs ship with the feature. Setup, boundaries, how-it-works, and this manual stay in `docs/`. Pre-lock research stays on the issue; after lock, land only the durable current contract and link the issue. Design deliverables close under [design handoff coverage](#design-handoff-coverage); issue closure or a design merge is not production adoption.

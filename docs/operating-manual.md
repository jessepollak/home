...
Start with six engineering issue lanes and up to eight pooled executing children. These are ceilings, not quotas: never start work merely to fill capacity. Each issue lane owns scope through implementation, review/proof, correction, and delivery; writers and reviewers consume pooled capacity rather than becoming separate lanes.

When eligible independent work exists, target four productive, non-overlapping implementation writers while retaining capacity for independent review and QA/publication. A lane waiting solely on provider, device, or Jesse evidence does not consume an implementation writer; its blocker and resumption trigger remain on GitHub.

The four-writer ceiling and two simultaneous full-build/browser-job ceiling are separate. Writers may scope, edit, and run focused non-browser checks while heavy jobs queue; browser checks still count against the heavy-job cap. CI watchers are not workers. Scale only to eight lanes and ten children after measured occupancy, handoff, review, backlog, and resource thresholds support it. Jesse-ready PRs enter his approval queue.

### Append-only publication and backlog admission

For an ordinary owned feature branch, inspect remote refs and divergence read-only first, preserve others' contributions, then push ordinary commits; publish the PR or status as a separate action. Append corrections to published history. Do not routinely rebase, amend, or force-rewrite published history (and never force-update `main`); local unpublished cleanup is permitted. A published-history rewrite requires Jesse's explicit exception. Do not put speculative destructive fallbacks in routine command batches or evade guards or permissions. Merge a base only when actually necessary to resolve a demonstrated divergence or dependency. Revalidate applicable local checks on the resulting head before pushing; current-head CI, review, and required proof remain gates for readiness, not prerequisites for the push that starts CI.

An admission exclusion must name the issue, exact blocked subtask and files, current live conflicting run/ref or concrete dependency, owner and observable unblock trigger, and whether an independent bounded slice can proceed. Broad path/category exclusions are insufficient. Historical owner labels and preserved PRs do not alone establish an active writer; verify the ownership handoff before reassignment. Distinguish an implementation blocker from a later provider, physical-device, or deployment proof gate; if an API is genuinely unknown, record that fact rather than speculating.
...
- Service each actionable completed delivery or integration handoff as soon as it becomes actionable: integrate and validate it at the destination, assign a concrete correction, or record an explicit truthful blocker with the current owner and a named observable resumption condition. Disposition or closure alone is not service, and deferral without that blocker and resumption condition is prohibited. After servicing the handoff, advance its lane and dispatch the next priority-ordered independent eligible work within the ceilings. Target completion-to-next-action within five minutes as a service target, not a review deadline or a gate bypass. Do not wait for all lanes, siblings, or handoffs to finish, and do not create a global wait-for-all barrier; unrelated active or non-actionable handoffs do not block rolling dispatch. Never bypass hotspot/dependency exclusions or replenish to a quota.
- Before a long wait, account for the actual implementation-writer, reviewer, and QA mix; then launch eligible work or record the required admission-exclusion evidence. Measure completion-to-next-action, not total agent count or recurring unchanged status announcements.
...
### Calibrated QA and evidence

Before capture, freeze a risk-based acceptance matrix separating financial/security behavioral assertions, required visual presentation states, and external provider/device gates. Map every required state or assertion to its evidence. Automate the complete agreed behavioral matrix and present a small primary visual set plus required supplementary coverage; do not require one human-reviewed screenshot for every automated permutation. Automated assertions do not replace explicitly required visual, physical-device, or live-provider evidence.

Use one complete handoff: capture → deterministic validation → independent review → publication. Reviewers return verified findings together; assign one correction writer and recheck the changed gate rather than unrelated accepted behavior. Reuse accepted QA/capture tooling; reopen frozen scope only for a verified acceptance defect or an explicit requirement.

Metadata-only PR-body or publication changes on an unchanged head and media receive metadata-only checks. Every head-changing push still requires fresh exact-tip After/ready proof under [UI PR previews](ui-pr-previews.md). Automate upload mappings, hashes, issue references, template preservation, and body limits. Seal completed per-capture packages, not their parent directories; never rewrite original capture provenance. Time targets never waive any financial, security, design, provider, physical-device, or other acceptance gate.

...
- `bun run factory:preflight` before a factory pilot run (pinned repository, `agent/*` branch, Jesse's worktree-local GitHub noreply identity, and secret-free worktree/ambient environment)
- `bun check`
- Chromium product smoke
- delivery automation tests (`bun run gates`: repository gate canaries, commit-range metadata policy, and PR-metadata algorithms; also run inside `bun check`)

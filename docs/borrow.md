...
- `GET /api/borrow` returns version `1`: configured opportunities, verified non-zero debt/collateral positions, owner/provider scope, and truthful complete/partial discovery. A failed market read is `unavailable`; it is never a zero balance or zero position. A separate additive version-`1` `lending` section carries verified supply opportunities and non-zero lender positions so Borrow clients cannot interpret supplied loan assets as debt.

...
The Drawer-backed sheets (Send, Add money, Save, Borrow, the Activity detail sheet, and sign-in) load through `deferSheet`; `motion` loads only when a pointer first enters the full Home mark, and the navigation indicator is a CSS transform. Base UI Combobox and floating-ui still reach the initial path through Account settings' `CountrySelect`, and `@adraffy/ens-normalize` through `client/transfers` → `shared/transfers/recipient-name`.

...
| `identity_verifications` | record | each consented Sumsub verification attempt for a customer, with at most one active row per provider and provider environment; signed webhooks trigger applicant readback, which provides the current review facts. Rows cannot be deleted ([identity verification](identity-verification.md)) |
| `identity_verification_events` | record | append-only Home-normalized review facts and provider attempt markers for each verification change, with source and consent version ([identity verification](identity-verification.md)) |
| `identity_request_limits` | record | per-customer session and hosted-link creation timestamps used for rate limiting ([identity verification](identity-verification.md)) |
| `customer_restrictions` | record | a restriction on a customer, such as a final identity rejection; it survives applicant removal and ends only through a recorded release ([identity verification](identity-verification.md)) |

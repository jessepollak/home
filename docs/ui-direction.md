...
- **Proposal and selection surface:** new design work is proposed, reviewed and selected on [Storybook review boards](design-system.md#review-boards) built from production components.
- **Figma (optional, historical baseline):** the [Home Figma file](https://www.figma.com/design/ixgttt6IurKynsvMJpLYDC/Home) holds earlier approved designs, such as `Home — final` below, and serves as a reference and sketch space. Its component mapping and comment loop are in the [Figma workflow](design-explorations/figma-workflow.md). It is not required for new proposals and does not synchronize automatically with React.

...
- `POST /api/actions/:id/status`: records only bounded wallet outcomes (`unknown`, `rejected`, `expired`, or `failed`); it cannot claim confirmation. `expired` / `rejected` may be written from claimed `submitting` or `unknown` only when the row has no `submissionId` / `transactionHash` / `userOperationHash`. `failed` stays `submitting` + reference-free. A later Check status recover of claimed `unknown` with no chain handle expires that row; a confirming handle is left open.

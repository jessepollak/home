...
`POST /api/client-performance` accepts closed `home-startup` and `home-auth-phase` reports. The startup report's only dimensions are route (`/` or `/dashboard`), terminal outcome (`ready`, `signed-out`, `unavailable`, or `timeout`), cache provenance (`restored`, `cold`, or `unknown`), and bounded integer phase durations. The restore-only auth report adds fixed hint (`none`, `cdp`, or `base`) and outcome (`signed-out`, `verified`, `unavailable`, or `timeout`) enums plus navigation-relative SDK activation, CDP initialization, native settlement, and terminal session timings. Its `outcome` is the first settled account state: a late CDP restore after a no-hint `signed-out` settlement does not emit a second event. Auth timings are rounded to 50 ms and capped at 30 seconds. The contract accepts no account, owner, wallet, subject, identity, email, address, URL, query, hash, exception, error text, provider payload, or arbitrary string data.

A durable, identity-free CDP restore marker makes that late-restore case exceptional for previously server-verified CDP sessions. A residual race remains when both that marker and the readable live cookie are absent: email sign-in can begin while an unhinted CDP refresh is still in flight, and a valid late restore can replace the in-progress email UX. Server verification and owner fencing remain the authentication and privacy boundaries. The current lifecycle represents a native restore outage as signed out with recovery copy, so its restore event also records the first-settled `signed-out` outcome; distinguishing that outage in telemetry is a follow-up, not authority to change the user state in this slice.

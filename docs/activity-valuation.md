...
| `no-recent-close` | No completed Codex close within 24 hours before the transfer |
...
- Codex closes are cached per process by chain, contract, hour bucket, and USD (`8453:{contract}:{hour}:USD`). One aliased `getBars` request per page queries up to 25 buckets, each from 24 hours and one bar before its hour start through its hour end with `countback: 8` and `removeEmptyBars: true`; countback may reach before `from` to retrieve sparse non-empty bars, while Home selects only bars completed by fetch time. Non-empty settled buckets are cached for 24 hours; recent buckets and empty results are cached for at most 60 seconds and only until the next 15-minute bar closes. On a failed refetch, an expired bounded-cache entry can supply its bars only if fetched at or after the start of the transfer's 15-minute bar; failure does not renew its freshness or enter the cache. FX rates are cached by pair and date: 24 hours for completed days and 60 seconds for the current day.
...
- Unpriced rows recover without a new transaction. When the loaded Activity pages contain a row that is `quote-unavailable`, `fx-unavailable`, or `no-recent-close` within an hour of the window end, the client refetches the whole loaded feed after 15 seconds, then 60 seconds, then 180 seconds: at most three batched attempts per owner, window, and currency, skipped while the tab is hidden. Rows never fetch individually, scrolling does not add attempts, `unknown-token` is never retried, and a refetch never replaces a known value with a transient unpriced result.

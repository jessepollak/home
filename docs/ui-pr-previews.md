...
- **Proposed:** the Storybook commit, commit-specific deployment URL, direct manager and canvas URLs, selected capture, and concise observable criteria. “Commit-specific” means later pushes cannot silently change the reviewed artifact. Label it unreviewed until Jesse leaves an actual [GitHub comment or review](operating-manual.md#factory-runs) tied to that revision. A concrete approval reference includes that Jesse link and all of the preceding fields; factory review or an unreviewed proposal is never approval.
